...
// We'll define these types here to avoid importing vite directly
type ViteDevServer = any;
type ViteLogger = any;

let viteLogger: ViteLogger | null = null;

// Lazy initialize the vite logger
async function getViteLogger() {
  if (!viteLogger) {
...
export function serveStatic(app: Express) {
  ...
  if (!fs.existsSync(distPath)) {
    // If the build directory doesn't exist, serve a simple API response
